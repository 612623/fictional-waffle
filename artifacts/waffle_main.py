...
origins = [
    # Common local dev origins (IPv4/IPv6) and explicit scheme/port for static file server
    "http://localhost:8080",
    "http://127.0.0.1:8080",
    "http://[::1]:8080",
    # Allow direct FastAPI access if frontend is ever served from same port
    "http://127.0.0.1:8000",
    "http://localhost:8000",
]
...
# Note: If you see binary-looking request logs like 'Bad request version ("\x16\x03\x01...")' on the simple
# Python http.server, that's a browser attempting an HTTPS (TLS) handshake against a plain HTTP endpoint.
# Ensure you load the page via http:// (not https://) and that any service worker / extension isn't forcing HTTPS.
...
def create_user(user_data: UserCreate, db: Session = Depends(get_db)):
    ...
    # DEBUG LOG: incoming payload (omit in production or guard with env flag)
    try:
        print(f"[DEBUG] create_user payload: first_name={user_data.first_name} last_name={user_data.last_name} email={user_data.email} role_id={user_data.role_id} hire_date={user_data.hire_date}")
    except Exception:
        pass
    ...
    if not role:
        print(f"[DEBUG] Invalid role_id received: {user_data.role_id}")
    ...
    try:
        ...
        print(f"[DEBUG] DB error on user insert: {exc}")
        raise
    return new_user
